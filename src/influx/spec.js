...
describe('influxdb', () => {
    ...
    describe('init', () => {
        ...
        it('gets the names of the databases and creates a `securityheaders` database if one does not already exist', async () => {

            influx.getDatabaseNames.mockResolvedValue(['database1', 'database2']);

            await init();

            expect(influx.createDatabase).toBeCalledWith('securityheaders');

        });

        it('gets the names of the databases and does not create a `securityheaders` database if one already exists', async () => {

            influx.getDatabaseNames.mockResolvedValue(['database1', 'securityheaders']);
